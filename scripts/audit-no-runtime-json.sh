...
frontend_fetches="$(
  run_rg "fetch\\([^)]*/${legacy_json}" app/js app/*.html 2>/dev/null
)"
...
nginx_static_storage="$(
  run_rg "alias\\s+/data|try_files\\s+[^;]*(/data|${legacy_json}|mymedialibrary\\.db|\\.secrets)|root\\s+/data" docker/nginx.conf 2>/dev/null
)"
report_failure "Forbidden nginx static serving from runtime storage" "$nginx_static_storage"

nginx_legacy_routes="$(
  run_rg "location\\s+(=|~|/).*(/recommendations\\.json|/library_inventory\\.json|/media_probe_cache\\.json|/providers_mapping\\.json|/providers_logo\\.json|/recommendations_rules\\.json|/config\\.json)" docker/nginx.conf 2>/dev/null
)"
...
conf_mounts="$(
  run_rg "\\./conf:/conf|:/conf(:|$)|/conf\\s+#|/conf\\s*$" compose.yaml docker README.md docs 2>/dev/null
)"
...
runtime_fallback_phrases="$(
  run_rg "falling back to JSON|fallback JSON|JSON fallback|fallback to .*\\.json" backend/scanner.py backend/repositories app/js docker 2>/dev/null
)"
